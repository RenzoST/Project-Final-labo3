  ...
  ordenEl.addEventListener("change", async (e) => {
    ...
    try {
      ...
      if (moviesToRender && moviesToRender.length > 0) {
        const moviePromises = moviesToRender.slice(0, 20).map(id => getMovieDetails(id));
        const movies = await Promise.all(moviePromises);
        const validMovies = movies.filter(movie => movie !== null);
        renderMovies(validMovies);
      } else {
        console.error("No se encontraron películas para la opción seleccionada.");
      }
      ...
      if (query !== "") {
        ...
        try {
          ...
          if (peliculasParaMostrar && peliculasParaMostrar.length > 0) {
            const moviePromises = peliculasParaMostrar.slice(0, 20).map(id => getMovieDetails(id));
            const peliculas = await Promise.all(moviePromises);
            const peliculasValidas = peliculas.filter(pelicula => pelicula !== null);
